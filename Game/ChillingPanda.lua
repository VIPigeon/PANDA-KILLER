...
function ChillingPanda:new(x, y)
    local object = {
        are_we_chilling = true,
        panda = Panda:new(x, y),
    }
...
function ChillingPanda:special_chilling_panda_update()
    if not self.are_we_chilling then
        self.panda:update()
    else
        self.panda.animation_controller:set_sprite(PANDA_SPRITES.chilling)
    end
end

function ChillingPanda:special_chilling_panda_draw()
    self.panda:draw()
end

ChillingPanda.__index = Panda
